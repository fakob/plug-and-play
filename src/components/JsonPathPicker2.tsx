...
})()(
  {
    1: [
      function (require, module, exports) {
        ...
        const JPPicker = window.JPPicker || {};
    ...
    2: [
      function (require, module, exports) {
            ...
            if (keyCount > 0) {
              ...
              for (const key in json) {
                if (json.hasOwnProperty(key)) {
                  ...
                  html +=
                    '<span class="pick-path" title="Pick path">&' +
                    options.pickerIcon +
                    ';</span>';
        ...
        function off(event, elem, callback, capture) {
          ...
          elemIntern =
            typeof elemIntern === 'string'
              ? document.querySelector(elemIntern)
              : elemIntern;
        ...
        function fireClick(node) {
          ...
          if (node.ownerDocument) {
            ...
            throw new Error(
              'Invalid node passed to fireEvent: '.concat(node.id)
            );
          }
        ...
        function PickPathHandler(elm) {
          ...
          for (let i = 0; i < $parentsList.length; i += 1) {
            let key = $parentsList[i].dataset.key;
            const keyType = $parentsList[i].dataset.keyType;
            ...
            if (
              ...
              const keyReplacementText =
                options.keyReplacementText === undefined
                  ? ''
                  : options.keyReplacementText;
          ...
          pathSegments = pathSegments.map(function (segment, idx) {
            ...
            const isKeyForbiddenInDotNotation =
        ...
        function jsonPathPicker(source, json, target, opt) {
          ...
          options.pathQuotesType =
            options.pathQuotesType !== undefined
              ? options.pathQuotesType
              : 'single'; // Transform to HTML

          options.pickerIcon = options.pickerIcon || '#x1f4cb';
          let html = json2html(json, options);
          if (isCollapsable(json))
            html = '<a href class="json-toggle"></a>'.concat(html); // Insert HTML in target DOM element
          ...
          if (!options.WithoutPicker) {
            ...
            document
              .querySelectorAll(
                "[id*='".concat(sourceSelector, "'] .pick-path")
              )
              .forEach(function (el) {
                return el.parentNode.removeChild(el);
              });
