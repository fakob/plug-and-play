...
Nodes are building blocks that can be connected together to perform various tasks. They can for example
...
See a [list of available nodes](https://docs.google.com/document/d/11dS2uk3qdvrVBHGdu0af5c3NmpCylHUrxrkcu4oNexU/edit#heading=h.94je4wsnhfom)

- To connect nodes, they have input and output `sockets` depending on their functionality
- To execute them (calculate the outputs) you can specify the node's `update behaviour` defining how/when they should update.

### What nodes are available?

See _Explore > Available nodes_
