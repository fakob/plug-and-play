...
export default class PPGraph {
  ...
  createNode<T extends PPNode = PPNode>(
    ...
    if (type === placeholderNode) {
      ...
      nodeConstructor = getAllNodeTypes(this)[name]?.constructor;
      ...
      name = type;
      nodeConstructor = getAllNodeTypes(this)[type]?.constructor;
    }
    ...
    if (!nodeConstructor) {
      ...
      name = type;
      nodeConstructor = getAllNodeTypes(this)['Placeholder']?.constructor;
      newArgs.name = type;
    }
  ...
  getNextLinkID = (): number => {
    return Object.values(this._links).reduce(
      (prev, link) => (link.id > prev ? link.id : prev),
      0
    );
  };
  ...
  async connect(
    ...
    //create link class
    const link: PPLink = new PPLink(this.getNextLinkID(), output, input);
