...
import { HighPerformanceTextureType } from '../datatypes/highPerformanceTextureType';
...
const textureOutputName = "Texture";
...
export class Shader extends DRAW_Base {
  ...
  protected getDefaultIO(): Socket[] {
    return [
      ...
      new Socket(SOCKET_TYPE.OUT, textureOutputName, new HighPerformanceTextureType()),
  ...
  protected drawOnContainer(
    ...
    if (typeof images == "object") {
      ...
      imageKeys.forEach(key => {
        // allow both b64 images and raw textures
        if (typeof images[key] == "string") {
          uniforms[key] = PIXI.Texture.from(images[key]);
          console.log("bazorgin");
        }
        else {
          console.log("running directly m8");
          uniforms[key] = images[key];
        }
      });
    }
    if (
    ...
    // this is a little bit hacky... but the performance gain makes something like this neccessary
    this.setOutputData(textureOutputName, texture.texture);
    setTimeout(r => {
      this.setOutputData(textureOutputName, texture.texture);
    }, 100);
